...
public class CodeScanBranchSupportDelegate implements BranchSupportDelegate {
    ...
    public ComponentDto createBranchComponent(DbSession dbSession, ComponentKey componentKey,
        ...
        if (componentKey.getBranch().filter((x) -> {
            return x.getName().equals(mainComponentBranchDto.getKey()) && x.getType() == mainComponentBranchDto
                    .getBranchType();
        }).isPresent()) {
    ...
    private static final class BranchComponent extends ComponentKey {
        ...
        public ComponentKey getMainBranchComponentKey() {
            return projectKey.equals(this.dbKey) ? this : new BranchComponent(this.projectKey, this.projectKey, null);
        }
        ...
        public int hashCode() {
            return Objects.hash(this.projectKey, this.dbKey, this.branch);
        }

        public String toString() {
            return "BranchComponent{key='" + this.projectKey + '\'' + ", dbKey='" + this.dbKey + '\'' + ", branch="
                    + this.branch + '}';
        }
    }
}
