        ...
        <DisableableSelectOption
          ...
          disableTooltipOverlay={() => (
            <FormattedMessage
              ...
              values={{
                link: (
                  <Link to={{ pathname: `/organizations/${props.data.organizationKey}/quality_gates/show/${props.data.label}` }}>
...
export default function ProjectQualityGateAppRenderer(props: ProjectQualityGateAppRendererProps) {
  const { organization, allQualityGates, currentQualityGate, loading, selectedQualityGateName, submitting } = props;
            ...
            {selectedQualityGate && !hasConditionOnNewCode(selectedQualityGate) && (
              <Alert className="abs-width-600 spacer-top" variant="warning">
                <FormattedMessage
                  ...
                  values={{
                    link: (
                      <Link to={{ pathname: `organizations/${organization.kee}/quality_gates/show/${selectedQualityGate.name}` }}>
                        {translate('project_quality_gate.no_condition.link')}
                      </Link>
                    ),
                  }}
