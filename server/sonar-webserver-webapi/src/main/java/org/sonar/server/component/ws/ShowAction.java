...
public class ShowAction implements ComponentsWsAction {
  ...
  private Components.Component.Builder toWsComponent(DbSession dbSession, ComponentDto component, @Nullable SnapshotDto lastAnalysis,
    ...
    if (isProjectOrApp(component)) {
      ...
      return projectOrAppToWsComponent(project, organizationDto, lastAnalysis).setNeedIssueSync(needIssueSync);
    }
    ...
    if (component.getCopyComponentUuid() != null) {
      String branch = dbClient.branchDao().selectByUuid(dbSession, component.getCopyComponentUuid())
        ...
        .orElse(null);
      return componentDtoToWsComponent(component, organizationDto, parentProject, lastAnalysis, true, branch, null)
    ...
    if (branchDto.isPresent() && !branchDto.get().isMain()) {
      return componentDtoToWsComponent(component, organizationDto, parentProject, lastAnalysis, false, request.branch, request.pullRequest)
        .setNeedIssueSync(needIssueSync);
    } else {
      return componentDtoToWsComponent(component, organizationDto, parentProject, lastAnalysis, true, null, null)
        .setNeedIssueSync(needIssueSync);
    }
  }
