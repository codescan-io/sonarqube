...
public interface UserPermissionMapper {
  ...
  List<UserPermissionDto> selectUserPermissionsByQueryAndUserUuids(@Param("query") PermissionQuery query,
      @Param("userUuids") Collection<String> userUuids);

  List<String> selectUserUuidsByQuery(@Param("query") PermissionQuery query);

  /**
   * Fetch user ids based on permission query and only in a specific scope (global
   * permissions only or entity permissions only)
   */
  List<String> selectUserUuidsByQueryAndScope(@Param("query") PermissionQuery query);

  /**
   * Count the number of distinct users returned by
   * {@link #selectUserUuidsByQuery(PermissionQuery)}
   * {@link PermissionQuery#getPageOffset()} and
   * {@link PermissionQuery#getPageSize()} are ignored.
   */
  ...
  /**
   * select id of users with at least one permission on the specified entity but
   * which do not have the specified permission.
   */
  Set<UserIdDto> selectUserIdsWithPermissionOnEntityBut(@Param("entityUuid") String entityUuid,
  ...
  List<String> selectEntityPermissionsOfUser(@Param("userUuid") String userUuid,
      @Param("entityUuid") String entityUuid);

  void deleteByOrganization(@Param("organizationUuid") String organizationUuid);

  void deleteOrganizationMemberPermissions(@Param("organizationUuid") String organizationUuid,
