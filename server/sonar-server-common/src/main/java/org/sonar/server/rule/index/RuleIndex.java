...
import static org.sonar.server.es.EsUtils.*;
...
public class RuleIndex {
  ...
  private static final Query SECURITY_IMPACT_AND_HOTSPOT_FILTER_V2 =
    ES8QueryHelper.boolQuery(b -> b.should(ES8QueryHelper.nestedQuery(FIELD_RULE_IMPACTS,
  ...
  public SearchIdResult<String> search(RuleQuery query, SearchOptions options) {
    ...
    Map<String, Query> filters = buildFilters(query);
  ...
  public Iterator<String> searchAll(RuleQuery query) {
    ...
    co.elastic.clients.elasticsearch.core.SearchRequest esSearch = sourceBuilder
      ...
      .scroll(tb -> tb.time(SCROLL_TIME_IN_MINUTES, TimeUnit.Minutes))
      .build();
  ...
  private static Query buildQuery(RuleQuery query) {
  ...
  private Map<String, Query> buildFilters(RuleQuery query) {
