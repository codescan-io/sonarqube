...
public class Sorting {
  ...
  public List<SortOptions> fill(String name, boolean asc) {
    List<Field> list = fields.get(name);
    checkArgument(!list.isEmpty(), "Bad sort field: %s", name);
    return doFill(list, asc);
  }

  public List<SortOptions> fillDefault() {
    return doFill(defaultFields, true);
  }

  private static List<SortOptions> doFill(List<Field> fields, boolean asc) {
    return fields.stream().map(field -> {
      boolean effectiveAsc = asc != field.reverse;
      SortOrder order = effectiveAsc ? SortOrder.Asc : SortOrder.Desc;
      boolean effectiveMissingLast = asc == field.missingLast;
      String missing = effectiveMissingLast ? "_last" : "_first";
      return SortOptions.of(b -> b
        .field(f -> f
          .field(field.name)
          .order(order)
          .missing(missing)
